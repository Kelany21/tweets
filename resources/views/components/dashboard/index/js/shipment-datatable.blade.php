    ...
    var KTDatatablesSearchOptionsAdvancedSearch = function () {
        ...
        var initTable1 = function () {
            var table = $('#kt_datatable').DataTable({
                ...
                extend: "pdfHtml5",
                language: {
                    url: '{{ asset('js/datatable-ar.json') }}'
                },
                "search": {
			...
			<'row'<'col-sm-12 col-md-5'i><'col-sm-12 col-md-7 dataTables_pager'lp>>`,
                ...
                pageLength: 10,
                searchDelay: 500,
                ajax: "{{ uA($data.'/get-rows'.(request()->has('order_id') ? '/'.request()->get('order_id') : "").'?delete='.request()->get('delete')) }}",
                method: "GET",
                        ...
                        render: function (data, type, full, meta) {
                            ...
                            if (!full.deleted_at) {
                                ...
                                @foreach($additionalActions as $additionalAction)
                                    var add_id = {{ $additionalAction['add_id'] }};
                                    out += `<a href="{{ $additionalAction['href'] }}` + (add_id ? data : "") + `" class="{{ $additionalAction['class'] ?? "btn btn-light-primary" }} btn-xs btn-clean btn-icon mr-2 ml-2" data-toggle="tooltip" data-theme="dark" title="{{ $additionalAction['title'] }}"><i class="{{ $additionalAction['icon'] }}"></i></a>`;
                                @endforeach
                            } else {
