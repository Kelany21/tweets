...
class Menu
{
    ...
    public static function renderVerMenu($item, $parent = null, $rec = 0, $singleItem = false)
    {
        ...
        if (isset($item['separator'])) {
            ...
            echo '<li class="menu-section ' . ($rec === 0 ? 'menu-section--first' : '') . '">
                <h4 class="menu-text">' . trans('admin.' . $item['section']) . '</h4>
            ...
            // Badge
            echo '<span class="menu-text">' . trans('admin.' . $item['title']) . '</span>';
            if (isset($item['label'])) {
    ...
    public static function isActiveVerMenuItem($item, $page, $rec = 0)
    {
        ...
        if (isset($item['page']) && Str::startsWith($page, app()->getLocale().$item['page'])) {
            return true;
        }
